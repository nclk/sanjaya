"""Integration tests targeting a live MSSQL / Azure SQL instance.

These tests are **skipped** unless the ``SANJAYA_MSSQL_URL`` environment
variable is set to a valid SQLAlchemy connection URL, e.g.::

    export SANJAYA_MSSQL_URL="mssql+pyodbc://user:pass@host/db?driver=ODBC+Driver+18+for+SQL+Server&TrustServerCertificate=yes"
...
Run with::
    ...
    SANJAYA_MSSQL_URL="..." pytest packages/sanjaya-sqlalchemy/ -k mssql -v
"""
...
# ---------------------------------------------------------------------------
# Skip the entire module when no MSSQL connection is available.
# ---------------------------------------------------------------------------

MSSQL_URL = os.environ.get("SANJAYA_MSSQL_URL")

pytestmark = pytest.mark.skipif(
    MSSQL_URL is None,
    reason="SANJAYA_MSSQL_URL not set — skipping MSSQL integration tests",
)
...
def mssql_engine() -> sa.engine.Engine:
    assert MSSQL_URL is not None
    return sa.create_engine(MSSQL_URL)


@pytest.fixture(scope="module")
def mssql_tables(mssql_engine: sa.engine.Engine) -> Generator[dict[str, sa.Table]]:
...
@pytest.fixture(scope="module")
def joined_selectable(mssql_tables: dict[str, sa.Table]) -> sa.SelectBase:
    """A three-table join returned as a ``SelectBase``.

    This is the pattern that triggered the original ``ColumnClause``
    / ``nullable`` errors — the subquery columns are ``ColumnClause``
    objects, not schema-bound ``Column`` objects.
    """
    orders = mssql_tables["orders"]
    customers = mssql_tables["customers"]
    products = mssql_tables["products"]
...
@pytest.fixture(scope="module")
def mssql_inferred_provider(
    mssql_engine: sa.engine.Engine,
    joined_selectable: sa.SelectBase,
) -> SQLAlchemyProvider:
    """Provider over a joined SelectBase with auto-inferred columns."""
    return SQLAlchemyProvider(
        key="mssql_orders_inferred",
        label="MSSQL Orders (Inferred)",
        engine=mssql_engine,
        selectable=joined_selectable,
        capabilities=DatasetCapabilities(pivot=True),
    )
...
class TestMssqlColumnInference:
    """Auto-inferred columns from a multi-table join SelectBase.

    This is the code path that produced the original
    ``ColumnClause has no attribute 'nullable'`` error.
    """

    def test_inferred_columns_match_expected(
        self, mssql_inferred_provider: SQLAlchemyProvider,
    ) -> None:
        cols = mssql_inferred_provider.get_columns()
...
class TestMssqlQuery:
    def test_select_all(self, mssql_provider: SQLAlchemyProvider) -> None:
        result = mssql_provider.query(
            ["order_id", "customer", "region", "amount"],
        )
        assert result.total == 8
        assert len(result.rows) == 8

    def test_limit_offset_no_sort(self, mssql_provider: SQLAlchemyProvider) -> None:
        """MSSQL OFFSET/FETCH requires ORDER BY — verify the fallback."""
        r1 = mssql_provider.query(
            ["order_id", "customer"], limit=3, offset=0,
        )
        r2 = mssql_provider.query(
    ...
    def test_limit_offset_with_sort(self, mssql_provider: SQLAlchemyProvider) -> None:
        r1 = mssql_provider.query(
            ["order_id"],
    ...
    def test_filter(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(column="region", operator=FilterOperator.EQ, value="US"),
        ])
        result = mssql_provider.query(["order_id", "customer"], filter_group=fg)
        assert result.total == 4  # Acme (2) + Initech (2)

    def test_filter_is_null(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(column="note", operator=FilterOperator.IS_NULL),
        ])
        result = mssql_provider.query(["order_id", "note"], filter_group=fg)
        assert result.total == 5

    def test_filter_is_not_null(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
    ...
    def test_filter_contains(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(
    ...
    def test_sort_desc(self, mssql_provider: SQLAlchemyProvider) -> None:
        result = mssql_provider.query(
            ["order_id", "amount"],
    ...
    def test_empty_result(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(
                column="region", operator=FilterOperator.EQ, value="NOWHERE",
            ),
        ])
        result = mssql_provider.query(["order_id"], filter_group=fg)
        assert result.total == 0
        assert result.rows == []

    def test_filter_neq(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(column="region", operator=FilterOperator.NEQ, value="US"),
        ])
        result = mssql_provider.query(["order_id", "region"], filter_group=fg)
        assert result.total == 4  # EU (2) + APAC (2)
        assert all(r["region"] != "US" for r in result.rows)

    def test_filter_gt_lt(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
        ...
        result = mssql_provider.query(["order_id", "amount"], filter_group=fg)
        # amount > 1000 AND < 4000 → 1500, 2000, 3000 = 3 rows
        assert result.total == 3
        assert all(1000 < r["amount"] < 4000 for r in result.rows)

    def test_filter_gte_lte(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
    ...
    def test_filter_between(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(
    ...
    def test_filter_in(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(
    ...
    def test_filter_startswith(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(
    ...
    def test_filter_endswith(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(
    ...
    def test_filter_or_combinator(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(
        ...
        result = mssql_provider.query(["order_id", "region", "customer"], filter_group=fg)
    ...
    def test_filter_negate_condition(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(
    ...
    def test_filter_negate_group(self, mssql_provider: SQLAlchemyProvider) -> None:
        """NOT(region = 'US') — tests MSSQL's NOT handling with NULLs."""
        fg = FilterGroup(
            negate=True,
            conditions=[
                FilterCondition(column="region", operator=FilterOperator.EQ, value="US"),
    ...
    def test_filter_nested_groups(self, mssql_provider: SQLAlchemyProvider) -> None:
        ...
        result = mssql_provider.query(["order_id", "region", "category"], filter_group=fg)
        assert result.total == 4  # All US orders are Hardware
        assert all(r["region"] == "US" for r in result.rows)

    def test_sort_asc(self, mssql_provider: SQLAlchemyProvider) -> None:
        result = mssql_provider.query(
            ["order_id", "amount"],
...
class TestMssqlSimpleAggregate:
    def test_group_by_region(self, mssql_provider: SQLAlchemyProvider) -> None:
        result = mssql_provider.aggregate(
    ...
    def test_group_by_two_columns(self, mssql_provider: SQLAlchemyProvider) -> None:
        result = mssql_provider.aggregate(
            group_by_rows=["region", "category"],
            group_by_cols=[],
            values=[ValueSpec(column="amount", agg=AggFunc.SUM)],
        )
        # US-Hardware, US-Services(none), EU-Hardware, EU-Services,
        # APAC-Hardware, APAC-Services
        assert result.total is not None
        assert result.total >= 4

    def test_multiple_measures(self, mssql_provider: SQLAlchemyProvider) -> None:
        result = mssql_provider.aggregate(
            group_by_rows=["region"],
            group_by_cols=[],
            values=[
    ...
    def test_aggregate_limit_offset_no_sort(
        self, mssql_provider: SQLAlchemyProvider,
    ) -> None:
        """MSSQL OFFSET/FETCH in aggregation path."""
        r1 = mssql_provider.aggregate(
            group_by_rows=["region"],
            group_by_cols=[],
            values=[ValueSpec(column="amount", agg=AggFunc.SUM)],
            limit=1,
    ...
    def test_aggregate_with_filter(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(
    ...
    def test_distinct_count(self, mssql_provider: SQLAlchemyProvider) -> None:
        result = mssql_provider.aggregate(
            group_by_rows=["region"],
            group_by_cols=[],
...
class TestMssqlPivotAggregate:
    def test_pivot_by_category(self, mssql_provider: SQLAlchemyProvider) -> None:
        result = mssql_provider.aggregate(
            group_by_rows=["region"],
    ...
    def test_pivot_limit_offset_no_sort(
        ...
        """MSSQL OFFSET/FETCH in pivot aggregation path."""
        r1 = mssql_provider.aggregate(
            group_by_rows=["region"],
            group_by_cols=["category"],
            values=[ValueSpec(column="amount", agg=AggFunc.SUM)],
    ...
    def test_multi_dim_pivot(self, mssql_provider: SQLAlchemyProvider) -> None:
    ...
    def test_pivot_with_filter(self, mssql_provider: SQLAlchemyProvider) -> None:
        fg = FilterGroup(conditions=[
            FilterCondition(
...
# ---------------------------------------------------------------------------
# Deferred / lazy init against MSSQL
# ---------------------------------------------------------------------------

class TestMssqlDeferredInit:
    def test_autoload_with_single_table(
        self, mssql_engine: sa.engine.Engine, mssql_tables: dict[str, sa.Table],
    ) -> None:
        """autoload_with=engine pattern against a real MSSQL table."""
        orders_table_name = mssql_tables["orders"].name

        p = SQLAlchemyProvider(
            key="mssql_autoload",
            label="MSSQL Autoloaded",
            engine=mssql_engine,
    ...
    def test_callable_selectable_with_join(
        ...
        tbl_names = {k: v.name for k, v in mssql_tables.items()}
...
class TestMssqlInferredProviderQueries:
    ...
    def test_flat_query(self, mssql_inferred_provider: SQLAlchemyProvider) -> None:
        cols = [c.name for c in mssql_inferred_provider.get_columns()]
        result = mssql_inferred_provider.query(cols)
        assert result.total == 8
        assert len(result.rows) == 8
    ...
    def test_flat_query_with_filter(
        ...
        result = mssql_inferred_provider.query(
    ...
    def test_flat_query_with_sort_and_pagination(
        ...
        r1 = mssql_inferred_provider.query(
            ["order_id"],
            sort=[SortSpec(column="order_id", direction=SortDirection.ASC)],
            limit=4, offset=0,
        )
        r2 = mssql_inferred_provider.query(
            ["order_id"],
            sort=[SortSpec(column="order_id", direction=SortDirection.ASC)],
            limit=4, offset=4,
        )
    ...
    def test_simple_aggregate(
        ...
        result = mssql_inferred_provider.aggregate(
            group_by_rows=["region"],
            group_by_cols=[],
            values=[ValueSpec(column="amount", agg=AggFunc.SUM)],
        )
